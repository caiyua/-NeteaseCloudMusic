<template>
  <div class="container">
    <van-pull-refresh v-model="isDown" @refresh="onRefresh">
      <!-- 下拉提示为空 -->
      <template #pulling></template>
      <template #loosing><span></span></template> <!-- 释放提示 -->
      <template #loading><span></span></template> <!-- 加载提示 -->
      <!-- 下拉提示为空 -->
        ...
        <div class="bg_circle">
          <div class="bg_df">
            ...
            <div class="songDetails">
              <div class="songLiImg">
                <img :src="recSongMenuDetails.coverImgUrl + '?param=128y128'">
              </div>
              <div class="songLiRi">
                <div class="songLi_title">{{recSongMenuDetails.name}}</div>
                <div class="creator_info">
                  <div class="creator_img">
                    <img :src="recSongMenuDetails.creator.avatarUrl + '?param=29y29'">
                  </div>
                  <div class="creator_nick">{{recSongMenuDetails.creator.nickname}}</div>
                </div>
                <!-- <div class="creator_tale">"{{recSongList.description}}"</div> -->
                <div class="creator_tale">"{{recSongMenuDetails.description}}"</div>
              </div>
            </div>
            <!-- 歌单收藏/评论/分享 -->
            <div class="song_interaction">
              <div class="interaction_box">
                <div class="song_collect">
                  ...
                  <span v-if="recSongMenuDetails.subscribers.length !== 0">收藏
                    {{recSongMenuDetails.subscribers.length}}</span>
                </div>
                <div class="song_comment">
                  <i class="iconfont icon-jianyi"></i>
                  <span v-if="recSongMenuDetails.commentCount !== 0">评论 {{recSongMenuDetails.commentCount}}</span>
                </div>
                <div class="song_share">
                  <i class="iconfont icon-fenxiang"></i>
                  <span v-if="recSongMenuDetails.shareCount !== 0">分享 {{recSongMenuDetails.shareCount}}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="songList">
        <div class="songLi_t">
          <div class="Li_Ti_l">
            ...
            <strong>播放全部</strong>
            <span>({{recSongMenuDetails.trackCount}})</span>
          </div>
          <div class="Li_Ti_r">
        ...
        <div class="songLi_row" v-for="(v,i) in recSongList_D" :key="i">
...
export default {
  name: 'RecSongList',
  computed: {
    ...mapState('recSongMenu', ['recSongList_D', 'recSongMenuDetails']) // 歌曲列表、歌单详情
  },
  data () {
    return {
      opacityStyle: {
